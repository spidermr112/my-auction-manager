...
if not df_filtered.empty:
    ...
    # 5컬럼으로 나누어 버튼을 작게 만들고 한 줄(좌우)에 배치
    ...
    with b_col3:
        st.markdown(f"<p style='text-align: center; font-size: 20px; font-weight: bold; margin-top: 10px;'>{st.session_state.current_idx + 1} / {total_count}</p>", unsafe_allow_html=True)
