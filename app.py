...
# 서비스 계정 JSON 정보
# [주의] private_key 내부의 \n이 파이썬 문자열로 처리될 때 발생하는 오류를 방지하기 위해 
# 데이터 전처리 로직을 get_gsheet_client에 추가했습니다.
...
def get_gsheet_client():
    ...
    # [핵심 수정 부분] 
    # PEM 파일을 읽을 때 발생하는 InvalidByte 오류는 대부분 개행 문자(\n) 처리 때문입니다.
    info = SERVICE_ACCOUNT_INFO.copy()
    
    # 1. 문자열 내의 실제 백슬래시+n 문자열을 실제 개행 문자로 치환
    # 2. 혹시 모를 앞뒤 공백 제거
