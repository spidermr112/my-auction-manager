...
st.caption("💡 표 왼쪽의 체크박스를 클릭하면 하단 상세 정보가 자동으로 바뀝니다.")

# [핵심 추가 기능] 행 선택 이벤트를 감지하는 data_editor
event = st.data_editor(
    df_filtered,
    use_container_width=True,
    hide_index=False,
    num_rows="dynamic",
    on_select="rerun",  # 선택 시 페이지 재실행하여 하단 데이터 업데이트
    selection_mode="single_row",  # 행 단위로 선택 가능하게 설정
    column_config={
...
if st.button("💾 목록 변경 사항 저장", use_container_width=True):
    st.session_state.df_list = event
...
# 선택된 행이 있는지 확인 (사용자가 표 왼쪽의 라디오버튼을 클릭했을 때)
selected_rows = event.selection.rows if hasattr(event, "selection") else []

if selected_rows:
    # 선택된 행의 데이터 추출
    idx = selected_rows[0]
    item_to_show = df_filtered.iloc[idx]
    ...
    with st.container(border=True):
        ...
        with c2:
            ...
            memo_content = item_to_show.get('특약사항', "내용 없음")
            # 선택된 매물의 고유 ID를 key로 주어 텍스트 영역을 갱신
            st.text_area("상세내용", value=memo_content, height=300, label_visibility="collapsed", key=f"memo_view_{idx}")
else:
    # 아무것도 선택하지 않았을 때는 첫 번째 매물을 기본으로 보여주거나 안내 메시지 출력
    if not df_filtered.empty:
        st.info("목록 왼쪽의 체크박스를 선택하면 상세 내용이 이곳에 표시됩니다.")
    else:
