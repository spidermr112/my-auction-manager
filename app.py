...
if not df_filtered.empty:
    # 1. 상세 내용을 볼 매물을 선택 (오류 없는 안전한 방식)
    options = {i: f"[{row['상태']}] {row['소재지']} ({row['고객명']})" for i, row in df_filtered.iterrows()}
    selected_idx = st.selectbox("🎯 하단 상세 확인을 위한 매물 선택", options.keys(), format_func=lambda x: options[x])
