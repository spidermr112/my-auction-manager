...
st.caption("💡 표 왼쪽의 버튼을 선택하면 하단에 상세 특약이 나타납니다.")

# [추가된 핵심 기능] st.data_editor에 selection_mode 추가
selected_rows = st.data_editor(
    df_filtered,
    use_container_width=True,
    hide_index=False,
    on_select="rerun",  # 선택 시 화면을 즉시 갱신하여 하단에 반영
    selection_mode="single_row",  # 한 행만 선택 가능
    column_config={
...
if st.button("💾 목록 변경 사항 저장", use_container_width=True):
    st.session_state.df_list = selected_rows
    st.toast("저장되었습니다.")

# 4. 선택 매물 상세 보기 (선택된 행에 따라 자동 연동)
st.markdown("---")
...
if hasattr(selected_rows, "selection") and selected_rows.selection.rows:
    ...
    item = df_filtered.iloc[selected_idx]
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            st.success(f"💰 **가액:** {item.get('가액', 0)} / **월세:** {item.get('월세', 0)}")
        with c2:
            st.markdown("**📜 상세 특약내용 및 비밀번호**")
            st.text_area("상세내용", value=item.get('특약사항', ""), height=300, label_visibility="collapsed")
else:
    st.info("목록에서 매물을 선택(왼쪽 버튼 클릭)하면 상세 내용이 여기에 표시됩니다.")
