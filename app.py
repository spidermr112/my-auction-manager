...
with col_reg:
    ...
    if st.button("🏠 데이터베이스 저장"):
        if address:
            # 중복 방지를 위한 고유 ID (시간밀리초 + 현재 행 개수)
            new_id = f"{int(time.time() * 1000)}_{len(st.session_state.data)}" 
...
with col_list:
    ...
    # 1. 상단 지표 요약
    active_count = len(st.session_state.data[st.session_state.data['status'] == "진행중"])
    st.info(f"현재 진행 중인 매물은 총 **{active_count}건**입니다.")

    search_query = st.text_input("🔍 키워드 검색 (주소/특약)", placeholder="검색어를 입력하세요.")
    st.write("---")
    ...
    f_sub = create_checkbox_filter("물건 소분류", all_subs)
        ...
        if active_df.empty:
            ...
            for i, row in active_df.iterrows():
                with st.expander(f"📍 {row['address']} ({row['item_sub_category']} / {row['price']}만원)"):
                    ...
                    with c1:
                        st.write(f"**종류:** {row['trade_type']} | **면적:** {row['area']} | **특약:** {row['description']}")
                    with c2:
                        # 중복 키 에러를 방지하기 위해 id와 인덱스를 결합
                        if st.button("거래완료", key=f"done_{row['id']}_{i}"):
    ...
    with tab2:
        ...
        if not done_df.empty:
            if st.button("완료 목록 영구 삭제", key="clear_all_done"):
