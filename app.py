...
# 1. 페이지 설정 및 [초슬림 콤팩트] CSS
st.set_page_config(page_title="페이지부동산", page_icon="📄", layout="wide")

st.markdown("""
    <style>
    /* 1. 내비게이션 바: 가로폭을 최소화하여 중앙 정렬 */
    div[data-testid="stHorizontalBlock"]:has(.nav-marker) {
        display: flex !important;
        flex-direction: row !important;
        justify-content: center !important; /* 중앙 모춤 */
        align-items: center !important;
        gap: 0px !important;
        width: fit-content !important; /* 가로폭을 내용물만큼만 */
        margin: 0 auto 10px auto !important; /* 화면 중앙 배치 및 하단 여백 */
        padding: 0 !important;
    }
    
    /* 2. 각 요소(버튼, 숫자) 사이의 간격 제거 */
    div[data-testid="stHorizontalBlock"]:has(.nav-marker) > div[data-testid="column"] {
        flex: 0 0 auto !important; /* 너비를 확장하지 않음 */
        width: auto !important;
        min-width: 0 !important;
        padding: 0 !important;
    }

    /* 3. 내비게이션 버튼: 가로폭과 세로폭 모두 슬림하게 */
    .stButton > button[key^="btn_nav_"] {
        ...
        padding: 0 12px !important; /* 좌우 여백 축소 */
        border-radius: 6px !important;
        box-shadow: 0 1px 2px rgba(0,0,0,0.05) !important;
    }
    
    /* 4. 중앙 숫자: 폰트 크기를 줄여 버튼과 일체감 형성 */
    .nav-counter {
    ...
    /* 5. 메모 저장 버튼: 가로폭을 꽉 채우지 않고 적당히 조절 */
