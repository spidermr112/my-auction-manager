...
if not df_filtered.empty:
    ...
    edited_df = st.data_editor(
        ...
        hide_index=True,
        column_config={
            "선택": st.column_config.CheckboxColumn("선택", help="하나만 선택 가능합니다."),
    ...
    # 4. 선택 매물 상세 특약 확인 (단일 선택 강제 로직)
    st.markdown("---")
    st.markdown("### 🔍 선택 매물 상세 특약 확인")

    # 체크된 행들 중에서 가장 마지막(최신)에 체크한 것 하나만 인덱스로 가져옴
    selected_indices = edited_df[edited_df["선택"] == True].index.tolist()

    if selected_indices:
        # **핵심**: 여러 개를 체크해도 리스트의 마지막 항목(최신 선택)만 표시
