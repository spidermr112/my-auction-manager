...
st.markdown("""
    <script>
    // 엔터 키를 눌렀을 때 다음 입력창으로 포커스를 이동시키는 시도
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            ...
            if (index > -1 && index < inputs.length - 1) {
                ...
                e.preventDefault(); // 엔터의 기본 제출 동작 방지
            }
        }
    });
    </script>
...
if not df.empty:
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "item_category": "대분류", "item_type": "물건종류", "price": "가액(만원)"
        },
